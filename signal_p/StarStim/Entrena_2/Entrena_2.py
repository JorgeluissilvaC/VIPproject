...
#%%
clf.fit(X_train_minmax[:,indm],LABELS)
clf2 = svm.SVC(kernel='linear', C=1)
scores = cross_val_score(clf2, X_train_minmax[:,indm], LABELS, cv=10)
scores.mean()
print "Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2)
#X_train_sc = scaler.fit_transform(X_train)
#
#scores = cross_val_score(lsvc, X_train_sc, LABELS, cv=10)
#scores.mean()
#print "Accuracy: %0.2f (+/- %0.2f) \n" % (scores.mean(), scores.std() * 2)
#clf.fit(X_train_sc,LABELS)
#
#
##
##clfg = GaussianNB()
##clfg.fit(X_train, LABELS)
##scores = cross_val_score(clfg, X_train, LABELS, cv=8)
##scores.mean()
##print "Accuracy: %0.2f (+/- %0.2f) \n" % (scores.mean(), scores.std() * 2)
...
if (dimen == 3):
    M_F = np.mean(S, axis=3) # Potencia promedio para cada frecuencia
    FEAT = np.reshape(M_F, (ff[0], M_F.shape[2]*ff[1]))

elif (dimen == 2):    
    M_F = np.mean(S, axis=2) # Potencia promedio para cada frecuencia
    FEAT = M_F
    FEAT = np.reshape(FEAT, (M_F.shape[0]*M_F.shape[1],1))
    FEAT = FEAT.T
    
Feat = min_max_scaler.transform(FEAT)
Feat2 = Feat[:,indm]
